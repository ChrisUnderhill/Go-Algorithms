...
def runMatch(evt, comp=False):
	...
	#otherwise tell the user to select two programs and print which programs have currently been selected.
	else:
		print("Please select two programs You have currently selected:")
		print(selectedPrograms)
	...
	def addProg(evt):
		...
		if len(name)>0 and len(loc)>0:
			try:
				...
				results.SetLabel("Please make sure the AI is a python file ending in .py")
		else:
...
def renameAI():
	...
	def saveChange(evt):
		...
		if len(progName.GetLineText(0))>0:
			programList[programIndex][0] = progName.GetLineText(0)			#open the repository in overwrite mode and loop through program list
